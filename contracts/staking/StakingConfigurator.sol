...
contract StakingConfigurator is VersionedInitializable, IStakingConfigurator {
  ...
  function getMultiFeeDistribution() external view override returns (address) {
    return getAddress(MULTI_FEE_DISTRIBUTION);
  }

  function setMultiFeeDistributionImpl(
    address impl,
  ...
  function getChefIncentivesController() external view override returns (address) {
    return getAddress(CHEF_INCENTIVES_CONTROLLER);
  }

  function setChefIncentivesControllerImpl(
    address impl,
    bytes calldata initParams
  ) external override onlyPoolAdmin {
  ...
  function getProtocolRevenueDistribution() external view override returns (address) {
    return getAddress(PROTOCOL_REVENUE_DISTRIBUTION);
  }

  function setProtocolRevenueDistributionImpl(
    address impl,
    bytes calldata initParams
  ) external override onlyPoolAdmin {
