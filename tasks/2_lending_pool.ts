  ...
  .setAction(async ({ verify, pool }, DRE: HardhatRuntimeEnvironment) => {
    try {
      ...
      if (!notFalsyOrZeroAddress(lendingPoolImplAddress)) {
        ...
        await waitForTx(sendContractTx(lendingPoolImpl, 'initialize', [addressesProvider.address]));
      }
      console.log('\tSetting lending pool implementation with address:', lendingPoolImplAddress);
      // Set lending pool impl to Address provider
      await waitForTx(
        sendContractTx(addressesProvider, 'setLendingPoolImpl', [lendingPoolImplAddress])
      );
      ...
      await waitForTx(
        sendContractTx(addressesProvider, 'setLendingPoolConfiguratorImpl', [
          lendingPoolConfiguratorImplAddress,
        ])
      );
      ...
      await waitForTx(sendContractTx(lendingPoolConfiguratorProxy, 'setPoolPause', [true]));
